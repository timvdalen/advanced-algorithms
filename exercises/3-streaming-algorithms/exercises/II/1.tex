...
\end{split}
\]
We can conclude that there exists a $c$ with $0 < c < 1$ for which the probability $\text{Pr}[|\tilde{\Phi}(\sigma) - \Phi(\sigma)| \geq c * \Phi(\sigma)]$ is less or equal to $\frac{1}{6}$.
	\item
\end{enumerate}
